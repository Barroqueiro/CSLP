...
class Preditor {
	private:
		...
		string fileName;
		int quantization/*! Number of shifts to encode */;
	...
	public: 
	    ...
	    */
	   	Preditor(int Type, int M,string file,int TypeVideo,int n_f,int flag,int bs,int ss,int peri,int q){
	   		fileName=file;
	   		if (flag == 1){// Encoding
	   			block_size = bs;
	   			search_space = ss;
	    	...
	    	*/
	    	...
	    	void encode_by_blocks(vector<Mat> planes){
	   		...
	   		Mat current_block;
	   		Mat possible_block;
	   		Mat replace_block(block_size,block_size,0);
	   		Mat min_block;
	   		int lastSum = 100000;
	   		int currentSum = 0;
	   		int d_x=0;
					...
					for(int i = 0; i < col; i+=block_size){
						...
						for(int n = 0; n < block_size; n++){
							for(int m = 0; m < block_size; m++){
								lastPixel = (int)min_block.at<uchar>(n,m);
								int z= (int)((int)current_block.at<uchar>(n,m) - (int)min_block.at<uchar>(n,m));
								pixel = (int)current_block.at<uchar>(n,m);
								//cout << "Pixel para dar encode" << (int)current_block.at<uchar>(n,m) << endl;
								if (z < 0){
									ruido = z * (-1);
									ruido = ruido >> quantization;
									ruido = ruido * (-1);
								}else{
									ruido = z >> quantization;
								}
								//cout << "Ruido: " << ruido << endl;
								g->encode(ruido);
								if (ruido < 0){
									ruido = ruido * (-1);
									ruido = ruido << quantization;
								...
								if (ruido + lastPixel < 0 || ruido + lastPixel > 255){
									cout << "Pixel" << pixel << endl;
									cout << "Verificação" << ruido + lastPixel << endl;
								}
								//cout << "Verificação" << ruido + lastPixel << endl;
								matrix.at<uchar>(c+n,i+m) = (unsigned char) (ruido+lastPixel);
					...
					for(int i = 0; i < colunas; i+=block_size){
						...
						for(int a = 0; a < block_size; a++){
							for(int b = 0; b < block_size; b++){
								...
								real = ruido + (int) current_block.at<uchar>(a,b);
								//cout << real << endl;
								if (real < 0 || real > 255){
									cout << "Verificação" << real << endl;
								}
								if (real < 0){
									real = abs(real);
								}
								...
								m1.at<uchar>(c+a,i+b) = (unsigned char)(real);							
							}
						}				
					}
				}
					...
					for(int i = 0; i < colunas; i+=block_size){
						...
						for(int a = 0; a < block_size; a++){
							for(int b = 0; b < block_size; b++){
								...
								m2.at<uchar>(c+a,i+b) = (unsigned char)(real);
							}
						}				
					}
				}
					...
					for(int i = 0; i < colunas; i+=block_size){
						...
						for(int a = 0; a < block_size; a++){
							for(int b = 0; b < block_size; b++){
								...
								m3.at<uchar>(c+a,i+b) = (unsigned char)(real);
							}
						}				
					}
				}
					...
					for(int i = 0; i < colunas/2; i+=block_size){
						...
						for(int a = 0; a < block_size; a++){
							for(int b = 0; b < block_size; b++){
								...
								temp2.at<uchar>(c+a,i+b) = (unsigned char)(real);
							}
						}				
					}
				}
					...
					for(int i = 0; i < colunas/2; i+=block_size){
						...
						for(int a = 0; a < block_size; a++){
							for(int b = 0; b < block_size; b++){
								...
								temp3.at<uchar>(c+a,i+b) = (unsigned char)(real);
							}
						}				
					}
				}
	   	...
	   	void encodeJPEG1(Mat f){
			...
			for(int c = 0; c < lin; c++){
				for(int i = 0; i < col; i++){
					...
					erro = pixel-lastPixel;
					if (erro < 0){
						ruido = erro * (-1);
						ruido = ruido >> quantization;
						ruido = ruido * (-1);
					}else{
						ruido = erro >> quantization;
					}
					...
					if (ruido < 0){
						ruido = ruido * (-1);
						shifted = ruido << quantization;
						shifted = shifted * (-1);
					}else{
						shifted = ruido << quantization;
					}
					real = shifted + lastPixel;
					//cout << "Real: " << real << endl; 
					f.at<uchar>(c,i) = (unsigned char) real;
	   	...
	   	void encodeJPEG4(Mat f){
			...
			for(int c = 0; c < lin; c++){
				for(int i = 0; i < col; i++){
					...
					f.at<uchar>(c,i) = (unsigned char) real;
					res.at<uchar>(c,i) = (unsigned char) (real);
					count++;
				}
			}
	   	}
	   	...
	   	void encodeJPEG6(Mat f){
			...
			for(int c = 0; c < lin; c++){
				for(int i = 0; i < col; i++){
					...
					if (erro < 0){
						ruido = erro * (-1);
						ruido = ((unsigned char) ruido) >> quantization;
						ruido = ruido * (-1);
					}else{
						ruido = erro >> quantization;
					}
					...
					if (ruido < 0){
						ruido = ruido * (-1);
						shifted = ((unsigned char) ruido) << quantization;
						shifted = shifted * (-1);
					}else{
						shifted = ruido << quantization;
					}
	   		...
	   		if (typeVideo == 0){
				...
				for(int c = 0; c < linhas; c++){
					for(int i = 0; i < colunas; i++){
						...
						//cout << "Erro: " << d << endl;
						//cout << "Real: " << lastPixel-d << endl;
						result.at<uchar>(c,i) = (unsigned char) (lastPixel+d);
						count++;				
					}
				}
				return result;
				...
				if (od % 3 == 0){
					...
					for(int c = 0; c < linhas; c++){
						for(int i = 0; i < colunas; i+=2){
							...
							exit(0);
							result.at<uchar>(c,i) = (unsigned char) (lastPixel+d);
							result.at<uchar>(c,i+1) = (unsigned char) (lastPixel+d);
							count++;				
						}
					}
					od++;
	   		...
	   		if (typeVideo == 0){
				...
				for(int c = 0; c < linhas; c++){
					for(int i = 0; i < colunas; i++){
						...
						result.at<uchar>(c,i) = (unsigned char) (real);
						count++;				
					}
				}
				return result;
				...
				if (od % 3 == 0){
					...
					for(int c = 0; c < linhas; c++){
						for(int i = 0; i < colunas; i+=2){
							...
							result.at<uchar>(c,i) = (unsigned char) (real);
							result.at<uchar>(c,i+1) = (unsigned char) (real);
							count++;				
						}
					}
					od++;
				...
				if (od % 3 == 0){
					...
					for(int c = 0; c < linhas; c+=2){
						for(int i = 0; i < colunas; i+=2){
							...
							result.at<uchar>(c,i) = (unsigned char) (real);
							result.at<uchar>(c,i+1) = (unsigned char) (real);
							result.at<uchar>(c+1,i) = (unsigned char) (real);
							result.at<uchar>(c+1,i+1) = (unsigned char) (real);
							count++;				
						}
					}
					od++;
